...
### 9. Claude HUD — 실시간 사용량 모니터링

Claude Code 터미널 상단에 **실시간 상태 표시줄(HUD)** 이 표시됩니다.
Anthropic API에서 직접 사용량 데이터를 가져오므로 정확합니다.

표시 항목:
- **모델명** — 현재 사용 중인 Claude 모델
- **컨텍스트 사용량** — 컨텍스트 윈도우 점유율
- **플랜 사용량** — 5시간/7일 기준 사용 퍼센티지
- **도구 활동** — 현재 실행 중인 도구 상태
- **에이전트 상태** — 활성 에이전트 정보
- **Todo 진행도** — 작업 목록 완료율

`install.sh`가 자동으로 HUD를 설치하고 설정합니다. 별도 작업 불필요.
