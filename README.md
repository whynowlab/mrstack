...
Mr.Stack은 **대화 내용을 분류해서 영구 저장**합니다.

```
~/claude-telegram/memory/
  people/          → 대화에서 언급된 사람 정보
  projects/        → 프로젝트별 진행상황, 기술 결정
  decisions/       → 내린 결정과 그 이유
  preferences/     → 내 선호도 (코딩 스타일, 도구, 언어)
  daily/           → 일일 요약 아카이브
  knowledge/       → 새로 알게 된 정보
  patterns/        → 작업 패턴 데이터 (interactions.jsonl)
```

**3시간마다** 최근 대화를 분석해서 메모리를 자동 업데이트합니다.
